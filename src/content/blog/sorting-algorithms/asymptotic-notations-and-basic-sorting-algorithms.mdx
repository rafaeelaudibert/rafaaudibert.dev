...
This is the first article, in a series of 3, on which we will talk about Sorting Algorithms with tons of charts. On this first one, we will also explore asymptotic complexity in general. We will also have a playground at the end of the article, running `async` JavaScript code live on this page, right in your browser. You will be able to experiment some stuff (check it later).

> The JavaScript code runs directly in your browser. The graphics data were generated with a code written in C, compiled with GCC version $$5.1.0$$ which can be downloaded in [this](/assets/zip/sorting-1.zip) zip file containing the CSV data used as input.
...
These algorithms were also "translated" to JavaScript, which you can play with in the table below. Everything runs directly in your browser, so there is no API call and no request timeout to worry about: you are free to throw a big array at BubbleSort and watch it suffer. It also means the timings are measured on a real clock on your own machine, so they will vary with whatever else your laptop happens to be doing, and they will be slower than the C ones used for the graphics above. Do try the same code locally, though, and you will see that asynchronous JavaScript makes the sorting algorithms REALLY faster when using Bubble or Insertion sort, although it makes the Shell algorithm slower (we can't process a lot of things at the same time, as we must make all the different sequence values of $h$ in strict order). Hope you enjoyed, and see you next time!
